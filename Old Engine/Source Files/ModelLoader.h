...
private:
	...
	void loadModel(string const& path)
	{
		...
		const aiScene* scene = importer.ReadFile(path, aiProcessPreset_TargetRealtime_Quality | aiProcess_Triangulate | aiProcess_FlipUVs);
		// check for errors
	...
	Mesh processMesh(aiMesh* mesh, const aiScene* scene)
	{
		...
		Textures roughnessMaps = loadMaterialTextures(material, AI_MATKEY_GLTF_PBRMETALLICROUGHNESS_METALLICROUGHNESS_TEXTURE, "texture_roughness");
		textures.insert(textures.end(), roughnessMaps);

		// 3. normal maps
		std::vector<Textures> normalMaps = loadMaterialTextures(material, aiTextureType_NORMALS, "texture_normal");
		textures.insert(textures.end(), normalMaps.begin(), normalMaps.end());
		// 4. height maps
		std::vector<Textures> heightMaps = loadMaterialTextures(material, aiTextureType_AMBIENT, "texture_ao");
	...
	Textures loadMaterialTextures(aiMaterial* mat, aiTextureType type, unsigned int index, string typeName)
	{
		Textures textures;
		cout << type << "   " << typeName << endl;
		aiString str;
	...
	void appendModel(Model* mod)
	{
		for (int i = 0; i < mod->meshes.size(); i++)
		{
			meshes.push_back(mod->meshes.at(i));
		}
	}
};
