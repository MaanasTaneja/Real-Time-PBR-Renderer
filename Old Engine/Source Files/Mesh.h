...
public:
	...
	// render the mesh
	void Draw(Shader *shader, bool noTextures)
	{
		...
		for (unsigned int i = 0; i < textures.size(); i++)
		{
			...
			if (noTextures == false)
			{
				if (strcmp(name.c_str(), "texture_roughness") == 0)
				{
					glActiveTexture(GL_TEXTURE3);
				}

				if (strcmp(name.c_str(), "texture_diffuse") == 0)
				{
					glActiveTexture(GL_TEXTURE0);
				}

				if (strcmp(name.c_str(), "texture_normal") == 0)
				{
					glActiveTexture(GL_TEXTURE1);
				}


				if (strcmp(name.c_str(), "texture_ao") == 0)
				{
					glActiveTexture(GL_TEXTURE4);
				}

				if (strcmp(name.c_str(), "texture_metallic") == 0)
				{
